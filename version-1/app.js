(function(){
  ...
  if (phoneBtn) {
    const codes = [43,51,51,54,51,48,51,49,54,48,57,56]; // "+33612345678"
    const e164 = String.fromCharCode(...codes); // "+33612345678"
    const national = '0' + e164.slice(3);      // "0612345678"
    const display = national.replace(/(\d{2})(?=\d)/g, '$1 ').trim(); // "06 12 34 56 78"
  ...
  const errs = form ? {
    ...
    missionType: document.getElementById('missionType-error'),
  ...
  function validateLastName(){
    const el = els.lastName; if (!el) return true;
    const v = (el.value||'').trim(); let msg='';
    if (!v) msg = "Le nom du contact est requis.";
    else if (el.minLength && v.length < el.minLength) msg = `Au moins ${el.minLength} caractères.`;
    setError('lastName', msg); el.setCustomValidity?.(msg); return !msg;
  }
  function validateFirstName(){
    const el = els.firstName; if (!el) return true;
    const v = (el.value||'').trim(); let msg='';
    if (!v) msg = "Le prénom du contact est requis.";
    else if (el.minLength && v.length < el.minLength) msg = `Au moins ${el.minLength} caractères.`;
    setError('firstName', msg); el.setCustomValidity?.(msg); return !msg;
  }
  function validateCompany(){
    ...
    setError('companyName', msg); el.setCustomValidity?.(msg); return !msg;
  }
  ...
  function validatePhone(){
    const el = els.phoneNumber; if (!el) return true;
    const v = (el.value||'').trim(); let msg='';
    if (v && !frPhonePattern.test(v)) msg = 'Numéro FR invalide (ex: 06 12 34 56 78 ou +33 6 12 34 56 78).';
    setError('phoneNumber', msg); el.setCustomValidity?.(msg); return !msg;
  }
  function validateMission(){
    ...
    setError('missionType', msg); el.setCustomValidity?.(msg); return !msg;
  }
  ...
  if (form) {
    ...
    els.lastName?.addEventListener('input', debounce(validateCompany, 180));
    els.firstName?.addEventListener('input', debounce(validateCompany, 180));
    els.companyName?.addEventListener('input', debounce(validateCompany, 180));
    els.email?.addEventListener('input', debounce(validateEmail, 220));
    els.phoneNumber?.addEventListener('input', debounce(validatePhone, 220));
    els.missionType?.addEventListener('change', validateMission);
    els.message?.addEventListener('input', debounce(validateMessage, 180));

    // Marquer les champs comme "touchés" au premier blur/change
    els.lastName?.addEventListener('blur', ()=>{ markTouched(els.lastName); validateLastName(); });
    els.firstName?.addEventListener('blur', ()=>{ markTouched(els.firstName); validateFirstName(); });
    els.companyName?.addEventListener('blur', ()=>{ markTouched(els.companyName); validateCompany(); });
    els.email?.addEventListener('blur', ()=>{ markTouched(els.email); validateEmail(); });
    els.phoneNumber?.addEventListener('blur', ()=>{ markTouched(els.phoneNumber); validatePhone(); });
    els.missionType?.addEventListener('change', ()=>{ markTouched(els.missionType); validateMission(); });
    els.message?.addEventListener('blur', ()=>{ markTouched(els.message); validateMessage(); });

    form.addEventListener('submit', function(e){
      e.preventDefault();
      const ok = [validateLastName(), validateFirstName(), validateCompany(), validateEmail(), validatePhone(), validateMission(), validateMessage()].every(Boolean);
      if (!ok){
  ...
  function sendToGoogleScript(data){
    const APPS_SCRIPT_URL ='https://script.google.com/macros/s/AKfycbySl5AKcoyXwR-DvVxYRDujAqX-hLk01FcnZHICMJL7wTdfI2_jRUcCkfTigKI902lISg/exec'
  ...
  (function(){
    ...
    ['lastName','firstName','companyName','email','phoneNumber'].forEach(id=>{ const el=document.getElementById(id); if (el) el.setAttribute('autocomplete','off'); });
    try { f.reset(); } catch(e) {}
    f.classList.remove('was-validated');
    document.querySelectorAll('#contactForm .form-group').forEach(g=>g.classList.remove('touched'));
    ['lastName-error','firstName-error','companyName-error','email-error','phoneNumber-error','missionType-error','message-error'].forEach(id=>{ const er=document.getElementById(id); if (er) er.textContent=''; });
    ['lastName','firstName','companyName','email','phoneNumber','missionType','message'].forEach(id=>{ const el=document.getElementById(id); if (el && el.setCustomValidity) el.setCustomValidity(''); });
